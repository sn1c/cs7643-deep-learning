...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        Z2 = S @ self.weights['W2'] + self.weights['b2']
        p = self.softmax(Z2)
        ...
        y_expanded = np.zeros_like(p)
        y_expanded[np.arange(n), y] = 1
        
        dloss_dZ2 = 1 / n * (p - y_expanded)
        dloss_dW2 = S.T @ dloss_dZ2
        dloss_db2 = np.sum(dloss_dZ2, axis=0)
        
        dloss_dS = dloss_dZ2 @ self.weights['W2'].T
        
        dloss_dZ1 = dloss_dS * self.sigmoid_dev(Z1)
        dloss_dW1 = X.T @ dloss_dZ1
        dloss_db1 = np.sum(dloss_dZ1, axis=0)

        self.gradients['W1'] = dloss_dW1
        self.gradients['b1'] = dloss_db1
        
        self.gradients['W2'] = dloss_dW2
        self.gradients['b2'] = dloss_db2
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
