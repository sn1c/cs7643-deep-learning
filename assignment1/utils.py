...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    #############################################################################
    if shuffle:
        data_with_labels = list(zip(data, label))
        
        random.shuffle(data_with_labels)
        
        data = [x[0] for x in data_with_labels]
        label = [x[1] for x in data_with_labels]
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    x = np.arange(len(train_loss_history)) + 1
    
    loss_ax.plot(x, train_loss_history, label="train")
    loss_ax.plot(x, valid_loss_history, label="valid")
    
    loss_ax.set_title("Loss Curve")
    loss_ax.set_xlabel("Epochs")
    loss_ax.set_ylabel("Loss")
    loss_ax.set_xticks(np.arange(0, len(train_loss_history) + 1, 5))
    ...
    x = np.arange(len(train_acc_history)) + 1
    
    acc_ax.plot(x, train_acc_history, label="train")
    acc_ax.plot(x, valid_acc_history, label="valid")
    
    acc_ax.set_title("Accuracy")
    acc_ax.set_xlabel("Epochs")
    acc_ax.set_ylabel("Accuracy")
    acc_ax.set_xticks(np.arange(0, len(train_acc_history) + 1, 5))
