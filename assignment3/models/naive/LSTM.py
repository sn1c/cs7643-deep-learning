...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for t in range(seq_len):
            x_t = x[:, t, :] # (batch, feature)
            
            i_t = nn.functional.sigmoid(x_t @ self.w_ii + self.b_ii + h_t @ self.w_hi + self.b_hi)
            f_t = nn.functional.sigmoid(x_t @ self.w_if + self.b_if + h_t @ self.w_hf + self.b_hf)
            g_t = nn.functional.tanh(x_t @ self.w_ig + self.b_ig + h_t @ self.w_hg + self.b_hg)
            o_t = nn.functional.sigmoid(x_t @ self.w_io + self.b_io + h_t @ self.w_ho + self.b_ho)
            c_t = f_t * c_t + i_t * g_t
