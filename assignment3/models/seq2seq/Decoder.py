...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout=0.2, model_type="RNN", attention=False):
        ...
        if attention:
            self.linear_attn = nn.Linear(self.emb_size + self.encoder_hidden_size, self.emb_size)
    ...
    def compute_attention(self, hidden, encoder_outputs):
        ...
        # numerator = hidden @ encoder_outputs.transpose(1, 2)
        # q_norm = torch.norm(hidden, dim=-1).unsqueeze(1)
        # k_norm = torch.norm(encoder_outputs, dim=-1).unsqueeze(1)
        
        # denominator = q_norm * k_norm
    ...
    def forward(self, input, hidden, encoder_outputs=None):
        ...
        if self.attention:
            ...
            attention_prob = self.compute_attention(h_t, encoder_outputs)
